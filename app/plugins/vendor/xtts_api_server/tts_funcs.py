...
class TTSWrapper:
    ...
    def get_models_list(self):
        ...
        return [
            name
            for name in entries
            if os.path.isdir(os.path.join(self.model_folder, name))
        ]

    def get_wav_header(
        self, channels: int = 1, sample_rate: int = 24000, width: int = 2
    ) -> bytes:
        wav_buf = io.BytesIO()
    ...
    def update_cache(self, text_params, file_name):
        ...
        try:
            ...
            if (
                os.path.exists(self.cache_file_path)
                and os.path.getsize(self.cache_file_path) > 0
            ):
    ...
    def load_model(self, load=True):
        ...
        if self.model_source != "api" and self.model_source != "apiManual":
            ...
            if self.lowvram == False:
                ...
                print("Pre-create latents for all current speakers")
    ...
    def switch_model(self, model_name):
        ...
        if model_name == self.model_version:
            raise InvalidSettingsError(
                "The model with this name is already loaded in memory"
            )
            return
    ...
    def get_or_create_latents(self, speaker_name, speaker_wav):
        if speaker_name not in self.latents_cache:
            print(f"creating latents for {speaker_name}: {speaker_wav}")
            (
                gpt_cond_latent,
                speaker_embedding,
            ) = self.model.get_conditioning_latents(speaker_wav)
    ...
    def set_speaker_folder(self, folder):
        if os.path.exists(folder) and os.path.isdir(folder):
            ...
            # self.create_directories()
            print(f"Speaker folder is set to {folder}")
        else:
    ...
    def set_out_folder(self, folder):
        if os.path.exists(folder) and os.path.isdir(folder):
            ...
            # self.create_directories()
            print(f"Output folder is set to {folder}")
        else:
            raise ValueError("Provided path is not a valid directory")
    ...
    def set_tts_settings(
        ...
        if not isinstance(length_penalty, float):
            raise InvalidSettingsError(
                "Length penalty must be a floating point number."
            )

        # Check repetition_penalty
        if not (0.1 <= repetition_penalty <= 10.0):
            raise InvalidSettingsError(
                "Repetition penalty must be between 0.1 and 10.0."
            )

        # Check top_p
        if not (0.01 <= top_p <= 1):
            raise InvalidSettingsError(
                "Top_p must be between 0.01 and 1 and must be a float."
            )
        ...
        if not (20 <= stream_chunk_size <= 400):
            raise InvalidSettingsError(
                "Stream chunk size must be an integer between 20 and 400."
            )

        # Check enable_text_splitting
        if not isinstance(enable_text_splitting, bool):
            raise InvalidSettingsError(
                "Enable text splitting must be either True or False."
            )
    ...
    # MAIN FUNC
    def process_tts_to_file(
        self,
        text,
        speaker_name_or_path,
        language,
