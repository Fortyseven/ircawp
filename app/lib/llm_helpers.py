...
finished image as if you were looking at it. You are not talking to the user and not talking to a renderer: you are
...
they want shown, every named object, every count, every stated colour and every stated
position. Copy their text strings character
...
## Output format
Only return the detailed description of the image in natural language. Do not include any additional commentary, explanations, or markup.
"""
...
SYSTEM_PROMPT_EDIT_MEDIA = """# Edit Prompt Enhancer — General (v2, 精简版)
...
Do not include any text except the final prompt; no greetings, no explanations, no markdown code fences.

The user's edit instruction to rewrite is:
"""

# "The user will ask for changes to the provided image. Your job is to take that request and shape it into a more accurate request based on the image in question. This new request will be passed to the AI editor model that will perform your revised request. Be descriptive about the request in relation to the content of the image. Describe what is in the image, and the spirit of it, as part of your revised request in order to help the editor precisely make the changes. Do not make broad generalizations, give specific, even repeated directions if necessary. Be verbose and exact. Only return the text of the request; do not add further commentary."
