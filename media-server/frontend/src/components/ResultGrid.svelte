...
{#if result}
  ...
  <div class="grid" class:multi={result.images.length > 1}>
    {#each result.images as img, i}
      <figure class="shot">
        <img src="data:image/png;base64,{img.b64_json}" alt={result.prompt} />
        <figcaption class="mono">
          {#if img.revised_prompt}
            <span class="revised" title={img.revised_prompt}>revised</span>
          {/if}
          <a
            class="dl"
