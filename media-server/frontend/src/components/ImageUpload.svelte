<script>
  ...
  async function addFiles(files) {
    ...
    const loadedImages = (await Promise.all(imageFiles.map(readFile))).filter(
      Boolean,
    );
...
>
  ...
  />
  <button type="button" class="dropzone" onclick={() => fileInput.click()}>
    <span class="mono">{images.length ? "add more images" : "drop or paste images, or browse"}</span>
  </button>
